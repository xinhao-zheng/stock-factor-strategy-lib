...
- **Parameter domain** — the module coerces `param` (`int`, `float`, tuple of `int`) and raises `ValueError` naming `param` when it falls outside the formula's domain; it never returns a silently degenerate column.
...
- **参数域** —— 模块对 `param` 做强制类型转换（`int`、`float`、`int` 元组），超出公式定义域时抛出带 `param` 字样的 `ValueError`，不静默返回退化的列。
